...
<aside class="bg-white sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
  <div class="sidenav-header">
    ...
    <a class="align-items-center d-flex m-0 navbar-brand text-wrap" href="{{ route('dashboard') }}">
        <img src="{{ asset('assets/img/pingvp-color.svg') }}" class="navbar-brand-img h-100" alt="...">
    </a>
  </div>
    ...
    @if(auth()->user() && auth()->user()->user_role == 1)
      <li class="nav-item mt-2">
        ...
        <a class="nav-link {{ (Str::startsWith(Request::path(), 'production-management') ? 'active' : '') }}" href="{{ url('production-management') }}">
            <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                <i style="font-size: 1rem;" class="fas fa-lg fa-list-ul ps-2 pe-2 text-center {{ (Str::startsWith(Request::path(), 'production-management') ? 'text-white' : 'text-dark') }} " aria-hidden="true"></i>
            </div>
            <span class="nav-link-text ms-1">Production management</span>
        </a>
      </li>
      <li class="nav-item pb-2">
          <a class="nav-link {{ (Str::startsWith(Request::path(), 'user-management') ? 'active' : '') }}" href="{{ url('user-management') }}">
              <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
